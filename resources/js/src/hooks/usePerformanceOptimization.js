...
const usePerformanceOptimization = () => {
  ...
  const calculatePerformanceLevel = (capabilities) => {
    ...
    if (capabilities.connection === 'slow-2g' || capabilities.connection === '2g') score -= 20;
    else if (capabilities.connection === '3g') score -= 10;
    else if (capabilities.connection === '4g') score -= 5;

    // GPU score (10% weight)
    if (capabilities.gpu && capabilities.gpu.toLowerCase().includes('intel')) {
  ...
  const getOptimizedSettings = (level, avgFps = 60) => {
    ...
    if (avgFps < preset.targetFps * 0.8) {
      ...
      return {
        ...preset,
        meteorCount: Math.max(3, Math.floor(preset.meteorCount * downgradeFactor)),
        glowIntensity: Math.max(0.1, preset.glowIntensity * downgradeFactor),
        trailLength: Math.max(2, Math.floor(preset.trailLength * downgradeFactor))
      };
    }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // Auto-upgrade if performance is consistently good
        ...
        const currentIndex = levels.indexOf(performanceLevel);
        if (currentIndex < levels.length - 1 && frameCount.current > 300) { // Only after 5 seconds
          setPerformanceLevel(levels[currentIndex + 1]);
        }
      }
    }, 5000);
